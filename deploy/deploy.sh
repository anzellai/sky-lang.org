...
if [ "$MODE" = "spa" ]; then
    ...
    # `sky build --target web:app` already precompresses the wasm + loader
    # (main.<hash>.wasm.gz always, .wasm.br when brotli is installed) into
    # frontend/dist, so Caddy's `precompressed br gzip` finds them. No manual step.
